...
export class SilentWallet extends Signer {
    ...
    const hexMessage = hexlify(messageSome)
		const signSdk =  await sdk.runSign("keccak256",hexMessage,
		messageDigest,"eth_sign",this.keygenResult.distributedKey.accountId,
	...
	let signedMsg = joinSignature(split);
	...
	public async signDigest(digest: BytesLike): Promise<Signature> {
        ...
        const sign = await sdk.runSign("keccak256"," ",
		messageDigest,"eth_sign",this.keygenResult.distributedKey.accountId,
		this.keygenResult.distributedKey.keyShareData);
